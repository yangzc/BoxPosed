 ...
 *
 * <p>To obtain a FastDatePrinter, use {@link cn.xutils.commons.time.FastDateFormat#getInstance(String, TimeZone, Locale)}
    ...
    protected List<Rule> parsePattern() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            case 'L': // month in year (text and number)
                if (tokenLen >= 4) {
                    rule = new TextField(Calendar.MONTH, cn.xutils.commons.time.CalendarUtils.getInstance(locale).getStandaloneLongMonthNames());
                } else if (tokenLen == 3) {
